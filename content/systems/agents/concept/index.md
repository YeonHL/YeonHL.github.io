...
강화 학습, 멀티 에이전트는 이전부터 있었던 개념임을 알 수 있었음, 기술이 발전하면서 이를 구현하는 수준과 깊이가 달라짐

에이전트 구현 과정에서도 목표만 전달하는 것보다 요청에 따라 스스로 계획을 세우도록 구성한 뒤, 계획을 따라 실행하도록 구성했을 때 더 높은 정확도를 가졌음.  

각 에이전트의 자율성을 어디까지 보장할 것인지 고민, 에이전트로 구성하는 것보다 LangGraph 기반 워크플로우로 구성하는 것이 안정성이 독보적

어느 데이터까지만 선별하여 전달해야 할지 고민, 사용자 요청에 따른 목표를 명확하게 수행할 수 있도록 보조하여 LLM 성능을 최대한 발휘 -> 글의 주어진 지능을 정교하게 조율하는 부분

사례: 데이터 기반의 사례가 많음, 데이터를 기반으로 판단, 분석 등을 수행하여 사용자에게 알리거나 초안 등 보조의 역할 / 사람이 매번 수행하기 어려운 활동이나 근거가 명확한 활동을 수행하는 것이 바람직

이전에 만든 에이전트는 자율성, 반응성, 주도성, 사회성은 구현했지만 학습 능력 구현 X
프로세스에서 생성된 데이터를 어떻게 다시 활용할 수 있을지 고민 (파인튜닝으로도 가능하겠지만 성능 고려 필요, 모델은 데이터를 해석할 수 있도록 학습하고, 확보한 데이터 중 필요한 내용만 전달하여 활용하는 등의 방법 고려)
에이전트의 데이터 확보 및 학습 반영을 통한 고도화로 비즈니스 가치 창출은 큰 차이점

다단계 추론, 맥락 이해 어려움: 에이전트의 추론 깊이를 축소할 수 있도록 보조, 맥락 
새로운 시나리오 실패 -> 파인튜닝이 필요한 이유

100%의 신뢰도를 보장하지 않기에 사용자의 확인이 필요한 보조 도구, 이를 구현해서 고객에게 직접 전달하는 것은 위험, 전문가의 보조 정도의 위치가 가장 이상적으로 생각 (이미 데이터 권한이 있는 사용자에게 활용할 수 있는 도구로 전달)
